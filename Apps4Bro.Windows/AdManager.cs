...
namespace Apps4Bro
{
    public class AdManager
    {
        ...
        private string FormatRequest()
        {
            ...
            //   {0} = App4Bro app ID
            //   {1} = package family name (analytics)
            //   {2} = language code
            //   {3} = SDK version
            //   {4} = platform name
            //   {5} = device advertising ID (may be empty)
            string analyticsName = "";
            try { analyticsName = Windows.ApplicationModel.Package.Current.Id.FamilyName; }
            catch { /* not available outside a UWP package context */ }
